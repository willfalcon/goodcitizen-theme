  ...
  if ( have_rows( 'rows' ) ) : while (have_rows( 'rows' ) ) : the_row();
    ...
    .gc-row-' . $rowNum . ' {';
      ...
      get_template_part( 'inc/row', 'margins' );
        // .gc-row-$rowNum { css block is closed inside row-margins template.
        // Following template calls open and close their own blocks.

        // Set padding for the column contents.
      get_template_part( 'inc/col', 'padding' );

        // Set margins for column images.
      get_template_part( 'inc/col', 'img' );
